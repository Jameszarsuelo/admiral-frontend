...
export async function upsertPlanning(planningData: IPlanningForm): Promise<IPlanningForm> {

    console.log(planningData);

    try {
        const response = planningData.id ?
            await api.put(`/planning/${planningData.id}`, planningData) : await api.post(`/planning`, planningData);
